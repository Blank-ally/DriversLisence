...
public class FirstNameUtility {
    private Map<String,Integer> firstNameCodeMap = new HashMap<>();
    ...
    public int encodeFirstName(String name,  String  middleInitial) throws MissingNameException {
/* If the first name is blank, throws a MissingNameException. If the first name appears in the first name map,
 returns the sum of the codes for the first name and middle initial. If not, it returns the sum of the codes for the
first initial and middle initial. If a person has no middle initial,
return only the first name or first initial code. */
if (name.equals("")) {
    throw new MissingNameException("First name");

}
else if (firstNameCodeMap.containsKey(name) && middleInitial != "") {

    return firstNameCodeMap.get(name) + middleInitialCodeMap.get(middleInitial);

} else if (firstInitialCodeMap.containsKey(name.charAt(0)) && middleInitial != "") {
    return firstInitialCodeMap.get(name.charAt(0)) + middleInitialCodeMap.get(middleInitial);

} else if (firstInitialCodeMap.containsKey(name.charAt(0))){
    return firstInitialCodeMap.get(name.charAt(0));
}
else if (firstNameCodeMap.containsKey(name)){
    return firstNameCodeMap.get(name);
}
else return 0;
    }
}
